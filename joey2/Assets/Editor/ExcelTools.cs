...
public class ExcelTools : EditorWindow
{
	...
	void OnGUI()
	{
		if (outputPath == null)
		{
			outputPath = Application.dataPath + "/Resources/Configs/";
		}
		DrawOptions();
		//DrawExport();
		DrawAns();
	...
	{
        // default path
        excelsPath = Application.dataPath + "/../../configs";

        GUILayout.BeginHorizontal();
		GUILayout.Label("路径名称:");  
		excelsPath = EditorGUILayout.TextField(excelsPath);  
		GUILayout.EndHorizontal();
		//Debug.Log (Application.dataPath);

		GUILayout.BeginHorizontal();

		if (GUILayout.Button("确定", GUILayout.MinWidth(100), GUILayout.MaxHeight(20)))
		{
	...
	void DrawAns()
	{
		...
		{
			EditorGUILayout.LabelField("该路径没有任何文件");
		}
		else
		{
			...
			{
				GUILayout.BeginHorizontal();
                GUILayout.Toggle(true, s.Substring(s.LastIndexOf("\\") + 1));
                GUILayout.EndHorizontal();
			}
	...
	void LoadExcelsFromPath()
	{
		if(excelList==null) excelList=new List<string>();
		excelList.Clear();
		//获取选中的对象

		string[] directories = Directory.GetFiles(excelsPath,"*.xlsx");
...
//		Debug.Log (directories.Length);
	...
	//	Debug.Log (excelList.Count + "list");
	}
	...
	//		{
 //               GUILayout.BeginHorizontal();
	//			GUILayout.Toggle(true, s.Substring(s.IndexOf(excelsPath) + excelsPath.Length));
	//			GUILayout.EndHorizontal();
	//		}
	//		GUILayout.EndScrollView();
	//		GUILayout.EndVertical();
	...
	private void Convert()
	{
		...
		foreach(string assetsPath in excelList)
		{
			...
			if(indexOfFormat==0)
			{
				string fileName = excelPath.Substring (excelsPath.Length);
				Debug.Log (fileName);
