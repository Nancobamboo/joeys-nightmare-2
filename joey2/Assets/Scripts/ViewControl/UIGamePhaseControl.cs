...
public partial class UIGamePhaseControl : YViewControl
{
	...
	void OnHPChanged(int hp)
	{
		...
		if (m_DataJoeyPlayer.playerMaxHealth > 0)
		{
			...
			if (ratio < 0.25f)
			{
				m_View.TextHeart.color = Color.red;
			}
			else if (ratio < 0.5f)
			{
				m_View.TextHeart.color = new Color(1f, 0.5f, 0f);
			}
			else
			{
				m_View.TextHeart.color = Color.white;
			}
		}
	...
	private async void AddBagCard(ECardType cardType, UICardSimpleControl cardControl, bool isMoveCard = false)
	{
	...
	private async UniTask RemoveBagCard(ECardType cardType, UICardSimpleControl cardControl)
	{
		...
		if (m_BagCardDict.TryGetValue(cardTypeInt, out List<UICardSimpleControl> cardList))
		{
			...
			else if (cardType == ECardType.attack && m_FistCardCache != null)
			{
				...
				m_FistCardCache.IsEffecting = true;
				float delayTime = m_FistCardCache.CardEffect?.OnBecomeTopOfPile() ?? 0.5f;
				await UniTask.WaitForSeconds(delayTime);
				m_FistCardCache.IsEffecting = false;
			}
			else
			{
	...
	async void TakeEnemyDamage(object[] paraArray)
	{
		...
		await UniTask.WaitForSeconds(delayTime, cancellationToken: default);
		...
		if (hasSlowAttack && enemyCardControl != null && enemyCardControl.gameObject.activeSelf && enemyCardControl.CardData.currentHealth > 0)
		{
			...
			if (enemyAttack > 0)
			{
				CancellationToken enemyToken = default;
			...
			enemyCardControl.CardEffect.Id == ECardEffectId.DodgeChicken)
		{
			...
			await UniTask.WaitForSeconds(0.3f, cancellationToken: default);
		...
		{
			await UniTask.WaitForSeconds(finishDelayTime, cancellationToken: default);
		}
		...
		if (!useFistCard)
		{
			...
			if (!shouldKeep)
			{
				...
				{
					await UniTask.WaitForSeconds(removeDelayTime, cancellationToken: default);
				}
			}
			else
			{
	...
	async UniTask TakePlayerDamageAsync(int enemyAttack, UICardSimpleControl enemyCardControl, int envIndex, CancellationToken cancellationToken = default, UICardSimpleControl attackCardControl = null)
	{
		...
		if (defenceCardControl != null)
		{
			...
			if (reflectDamage > 0)
			{
				await AttackSpecialEnemy(enemyCardControl, reflectDamage, envIndex, default);
			}
